...
describe('Home Page', () => {
	...
	it('Member current Tiny Step has not been tracked in greater than 4 days and card tracking alert displays', () => {
		// Validate UI elements of the card alert
		...
		homePageTinySteps
			...
			.then(rows => {
				...
				cy.wrap(rows).eq(0).find('.card-status').should('not.exist')
				cy.wrap(rows)
					.eq(1)
					.then(row => {
