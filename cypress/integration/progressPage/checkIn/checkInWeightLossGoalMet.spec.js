...
describe('Progress Page', () => {
	...
	it('Member checks in, meets the weight loss goal, receives success message, and confetti animation', () => {
		...
		progressPageCheckIn.getGoalCheckInValueField().type('120')
		...
		cy.get('@goalPostInfo').then(xhr => {
			expect(xhr.request.body.currentValue).to.equal(120)
			expect(xhr.request.body.calculatedCurrentChange).to.equal(-20)
		})
		progressPageGoals.getCircleProgressPercent().should('equal', '100')
		progressPageCheckIn
